...
public class Tetrad
{
	...
	public Tetrad(BoundedGrid<Block> grid)
	{
		...
		if (tetradRandomizer == 0) { // Red "I"
			...
			Location[] tempLocations = {new Location(0, 3), new Location(0, 4), new Location(0, 5), new Location(0, 6)};
			blockLocations = tempLocations;
		} else if (tetradRandomizer == 1) { // Gray "T"
			tetradColor = Color.GRAY;
			Location[] tempLocations = {new Location(0, 3), new Location(0, 4), new Location(0, 5), new Location(1, 4)};
			blockLocations = tempLocations;
			...
			Location[] tempLocations = {new Location(0, 4), new Location(0, 5), new Location (0, 6), new Location(1, 6)};
			blockLocations = tempLocations;
		} else if (tetradRandomizer == 5) { // Blue "S"
			tetradColor = Color.BLUE;
			Location[] tempLocations = {new Location(1, 3), new Location(1, 4), new Location(0, 4), new Location(0, 5)};
			blockLocations = tempLocations;
		} else { // If tetradRadomizer is 6, then create a green "Z"
			tetradColor = Color.GREEN;
			Location[] tempLocations = {new Location(0, 3), new Location(0, 4), new Location(1, 4), new Location(1, 5)};
			blockLocations = tempLocations;
		} 
	...
	{
		//throw new RuntimeException("INSERT MISSING CODE HERE");
		Location[] originalTetradLocations = new Location[blocks.length];
		Location[] newTetradLocations = new Location[blocks.length];
		for (int i = 0; i < blocks.length; i++) {
			Location blockLocation = blocks[i].getLocation();
			originalTetradLocations[i] = blockLocation;
	...
	public Location[] removeBlocks()
	{
		...
		for (int i = 0; i < blocks.length; i++) {
			...
			grid.remove(blocks[i].getLocation());
		}
